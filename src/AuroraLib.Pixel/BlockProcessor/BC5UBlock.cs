...
namespace AuroraLib.Pixel.BlockProcessor
{
    ...
    /// </summary>
    public sealed class BC5UBlock : IBlockProcessor<IA<byte>>
    {
        ...
        private readonly BC4UBlock<IA<byte>> BC4 = new BC4UBlock<IA<byte>>();
        private readonly BC4UBlock<AToI<IA<byte>, byte>> BC4a = new BC4UBlock<AToI<IA<byte>, byte>>();
        ...
        public void DecodeBlock(ReadOnlySpan<byte> source, Span<IA<byte>> target, int stride)
        {
            ...
            Span<AToI<IA<byte>, byte>> intensity = MemoryMarshal.Cast<IA<byte>, AToI<IA<byte>, byte>>(target);
        ...
        public void EncodeBlock(ReadOnlySpan<IA<byte>> source, Span<byte> target, int stride)
        {
            ...
            ReadOnlySpan<AToI<IA<byte>, byte>> intensity = MemoryMarshal.Cast<IA<byte>, AToI<IA<byte>, byte>>(source);
