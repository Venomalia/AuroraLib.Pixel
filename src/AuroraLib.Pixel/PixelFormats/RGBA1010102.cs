...
namespace AuroraLib.Pixel.PixelFormats
{
    ...
    public struct RGBA1010102 : IRGBA<ushort>, IAlpha<byte>, IColor<RGBA1010102>
    {
        ...
        public static readonly PixelFormatInfo FormatInfo = new PixelFormatInfo(32, 10, 0, 10, 10, 10, 20, 2, 30);
        ...
        private static uint PackNativRGBA(int R, int G, int B, int A)
            => (uint)R | ((uint)(G) << 10) | ((uint)(B) << 20) | ((uint)(A) << 30);

        /// <inheritdoc/>
        public Vector4 ToScaledVector4() => new Vector4(PackedValue & 0b1111111111, (PackedValue >> 10) & 0b1111111111, (PackedValue >> 20) & 0b1111111111, (PackedValue >> 30) & 0b11) / Multiplier;
