...
namespace AuroraLib.Pixel.PixelFormats
{
    ...
    public struct IA8 : IIntensity<byte>, IColor<IA8>, IAlpha<byte>
    {
        ...
        private static byte PackIA4(byte intensity, byte alpha) => (byte)(alpha & 0xF0 | intensity >> 4);
        ...
        public void From8Bit<TColor>(TColor value) where TColor : IRGB<byte>
            => PackedValue = PackIA4(Help.BT709Luminance(value.R,value.G,value.B), value.A);

        /// <inheritdoc/>
        public void From16Bit<TColor>(TColor value) where TColor : IRGB<ushort>
            => PackedValue = PackIA4((byte)(Help.BT709Luminance(value.R, value.G, value.B) >> 8), (byte)(value.A >> 8));
