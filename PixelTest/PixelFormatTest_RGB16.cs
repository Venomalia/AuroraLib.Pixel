...
namespace PixelTest
{
    ...
    public class PixelFormatTest_RGB16
    {
        ...
        {
            IEnumerable<Type> availableAlgorithmTypes = AppDomain.CurrentDomain.GetAssemblies().SelectMany(x => x.GetTypes().Where(s => typeof(IRGB<ushort>).IsAssignableFrom(s) && !s.IsInterface && !s.IsAbstract));
